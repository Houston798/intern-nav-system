...
router.get('/:internId', authMiddleware, async (req: AuthRequest, res: Response) => {
...
router.post('/', authMiddleware, async (req: AuthRequest, res: Response) => {
  ...
  const intern = await db.prepare('SELECT id, mentor_id, department FROM users WHERE id = ? AND role = ?').get(internId, 'intern') as any
  if (!intern) return res.status(404).json({ error: '实习生不存在' })
  ...
  const created = await db.prepare('SELECT * FROM mentor_tasks WHERE id = ?').get(id) as MentorTaskRow
  res.status(201).json(created)
})

router.put('/:id', authMiddleware, async (req: AuthRequest, res: Response) => {
  const existing = await db.prepare('SELECT * FROM mentor_tasks WHERE id = ?').get(req.params.id) as MentorTaskRow
  ...
  if (role !== 'hr' && existing.mentor_id !== userId) {
    if (role === 'intern' && existing.intern_id === userId) {
      ...
      await db.prepare(
        "UPDATE mentor_tasks SET status = ?, updated_at = NOW() WHERE id = ?"
      ).run(status, req.params.id)

      const updated = await db.prepare('SELECT * FROM mentor_tasks WHERE id = ?').get(req.params.id)
...
router.delete('/:id', authMiddleware, async (req: AuthRequest, res: Response) => {
  ...
  await db.prepare('DELETE FROM mentor_tasks WHERE id = ?').run(req.params.id)
