...
// ── 通用进度更新 ──
router.post('/step', authMiddleware, async (req: AuthRequest, res) => {
  const userId = req.user?.id
...
// ── Step 2: MBTI 测评 ──
router.post('/mbti', authMiddleware, async (req: AuthRequest, res) => {
  const userId = req.user?.id
  ...
  await db.prepare('UPDATE users SET mbti_type = ? WHERE id = ?').run(mbti_type, userId)

  const existing = await db.prepare('SELECT steps_data FROM onboarding_progress WHERE user_id = ?').get(userId) as any
  ...
  await db.prepare(
    `INSERT INTO onboarding_progress (user_id, current_step, steps_data) VALUES (?, 3, ?)
     ON CONFLICT (user_id) DO UPDATE SET current_step=GREATEST(onboarding_progress.current_step, 3), steps_data=?`
...
// ── Step 3: 部门选择 ──
router.post('/department', authMiddleware, async (req: AuthRequest, res) => {
  const userId = req.user?.id
  ...
  await db.prepare('UPDATE users SET department = ? WHERE id = ?').run(department, userId)
  ...
  await db.prepare(
    `INSERT INTO onboarding_progress (user_id, current_step, steps_data) VALUES (?, 4, ?)
     ON CONFLICT (user_id) DO UPDATE SET current_step=GREATEST(onboarding_progress.current_step, 4), steps_data=?`
  ).run(userId, JSON.stringify(stepsData), JSON.stringify(stepsData))
...
// ── Step 4: 选择导师 ──
router.post('/mentor', authMiddleware, async (req: AuthRequest, res) => {
  const userId = req.user?.id
  ...
  const mentor = await db.prepare("SELECT id, name FROM users WHERE id = ? AND role = 'mentor'").get(mentor_id) as any
  if (!mentor) {
  ...
  await db.prepare(
    `INSERT INTO onboarding_progress (user_id, current_step, steps_data) VALUES (?, 5, ?)
     ON CONFLICT (user_id) DO UPDATE SET current_step=GREATEST(onboarding_progress.current_step, 5), steps_data=?`
  ).run(userId, JSON.stringify(stepsData), JSON.stringify(stepsData))
...
router.post('/goals', authMiddleware, async (req: AuthRequest, res) => {
  ...
  await db.prepare(
    `INSERT INTO onboarding_progress (user_id, current_step, steps_data) VALUES (?, 6, ?)
     ON CONFLICT (user_id) DO UPDATE SET current_step=GREATEST(onboarding_progress.current_step, 6), steps_data=?`
  ).run(userId, JSON.stringify(stepsData), JSON.stringify(stepsData))
...
// ── Step 6: 入职培训完成 ──
router.post('/training', authMiddleware, async (req: AuthRequest, res) => {
  const userId = req.user?.id
